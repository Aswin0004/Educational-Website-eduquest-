...
if (isset($_POST['login'])) {
    ...
    $stmt = $connect->prepare("SELECT * FROM staff WHERE email = ?");
    ...
    if ($result->num_rows === 1) {
        ...
        if (password_verify($password, $user['password'])) {
            // Valid email & password for staff
            $_SESSION['logged_in'] = true;
            $_SESSION['user_email'] = $email;
            $_SESSION['user_type'] = 'staff';  // User type is staff
            $_SESSION['user_name'] = $user['first_name'] . ' ' . $user['last_name']; 

            echo "<script>alert('Staff login successful!'); window.location.href='../admin/staff.php';</script>";
            exit();
        } else {
...
<body>
  <div class="form-container">
    ...
    <form action="staff_login.php" method="post">
        ...
        <a href="teacher_login.php">Login as teacher</a><br>
